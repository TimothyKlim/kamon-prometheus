...
metrics into a Prometheus-friendly format and provides integrations for
Akka HTTP so that your application can serve metrics to Prometheus.
...
* Provides Akka HTTP route that supports:
...
to <<Usage,include the extension’s Akka HTTP routes somewhere in your
...
1. Using the Akka HTTP endpoint
2. Collecting metrics via an Actor
...
------------------------------------------------------------------------------
import akka.actor.ActoorSystem
